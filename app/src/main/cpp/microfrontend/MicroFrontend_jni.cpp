...
// approach.
//
// JNI entry points target com.teya.agent.voice.MicroFrontend (the Kotlin
// wrapper — not written by this change; a separate step owns it) rather than
...
Java_com_teya_agent_voice_MicroFrontend_nativeCreate(
...
Java_com_teya_agent_voice_MicroFrontend_nativeDestroy(
...
Java_com_teya_agent_voice_MicroFrontend_nativeProcessSamples(
...
JNIEXPORT void JNICALL
Java_com_teya_agent_voice_MicroFrontend_nativeReset(
    JNIEnv* env, jclass clazz, jlong handle) {
    if (handle == 0) {
        return;
    }
